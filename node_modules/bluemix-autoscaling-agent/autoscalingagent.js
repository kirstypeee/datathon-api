...
var isTest = true;
var testServerUrl = '';
var isAgentEnabled = true;
var enabledMetrics = new Set(["CPU", "Memory", "NODEJSHeapMemory", "ResponseTime", "EventLoopLatency", "Throughput"]);
...
function parseUrlToHostPort() {
  ...
  // console.log(host + 'index of cdl.ibm.com'+host.indexOf('cdl.ibm.com'));
  if (host.indexOf('cdl.ibm.com') >= 0) {
    port = 80;
  }
  result.push(host);
  result.push(port);
...
function sendMetrics(object) {
  ...
  req.on('error', function(e) {
    // TODO: handle error.
    console.log('-----error in sendMetrics-------', e);
  });
  req.write(jsonStr);
  req.end();
}
...
function notifyStatus() {
  ...
  req.on('error', function(e) {
    // TODO: handle error.
    console.log('-----error in notifyStatus-------', e);
  });
  req.end();
}
...
function refreshHeartBeat() {
  ...
  var req = http.request(options, function(res) {
    ...
    res.on('end', function() {
      // console.log('-----config resBody-----',responseString);
      refreshConfigration(responseString);
    });
  });
  req.on('error', function(e) {
    // TODO: handle error.
    console.log('-----error in refreshHeartBeat-------', e);
  });
  req.end();
}

function resetReportInterval(newReportInterval) {
  clearInterval(reportIntervalID);
  intervalTime = newReportInterval;
  reportIntervalID = setInterval(function() {
    reportJob();
  }, intervalTime);
}

function refreshConfigration(configString) {
  var configObject;
  try {
    configObject = JSON.parse(configString);
  } catch (e) {
  	console.error("Failed to parse heart beat response:" + configString);
  	console.error("Error:" + e);
  	return
  }
  if ("reportInterval" in configObject) {
    reportInterval = parseInt(configObject["reportInterval"], 10) * 1000; //reportInterval from server is in unit Second and here we need ms
    if (reportInterval != intervalTime) {
      console.log("we will change report interval from " + intervalTime + " to " + reportInterval);
      resetReportInterval(reportInterval);
    }
  }
    ...
    if ("agent" in metricsConfig) {
      ...
      if (agent_config.length > 0) {
        ...
        for (var index in agent_config) {
          enabledMetrics.add(agent_config[index]);
        }
        // console.log("new enabledMetrics: " + JSON.stringify(Array.from(enabledMetrics)));
      } else {
        console.log("agent is empty in metricsConfig");
        isAgentEnabled = false;
      }
    } else {
      console.log("agent is not in metricsConfig");
      isAgentEnabled = false;
    }
  } else {
    console.log("metricsConfig is not in config");
    isAgentEnabled = false;
  }
}

function shouldReportCPU() {
  return (enabledMetrics.size > 0) && (enabledMetrics.has("CPU"));
}

function shouldReportMemory() {
  return (enabledMetrics.size > 0) && (enabledMetrics.has("Memory"));
}

function shouldReportHeap() {
  return (enabledMetrics.size > 0) && (enabledMetrics.has("NODEJSHeapMemory"));
}

function shouldReportResponseTime() {
  return (enabledMetrics.size > 0) && (enabledMetrics.has("ResponseTime"));
}

function shouldReportEventLoopLatency() {
  return (enabledMetrics.size > 0) && (enabledMetrics.has("EventLoopLatency"));
}

function shouldReportThroughput() {
  return (enabledMetrics.size > 0) && (enabledMetrics.has("Throughput"));
}
...
function calculate() {
  ...
  if (throughputStats.requestCount > 0) {
    ...
    var duration = curTime - throughputStats.lastCalculateTime;
...
function constructMetrics() {
  ...
  if (shouldReportHeap()) {
    ...
    // console.log("Heap memory is disabled");
  }

  if (shouldReportResponseTime()) {
    metrics.push(responseTimeObj);
  } else {
    // console.log("ResponseTime is disabled");
  }

  if (shouldReportEventLoopLatency()) {
    metrics.push(eventLoopLatencyObj);
  } else {
    // console.log("EventLoopLatency is disabled");
  }
  if (shouldReportThroughput()) {
    metrics.push(throughputObj);
  } else {
    // console.log("Throughput is disabled");
  }
...
function reportJob() {
  ...
  if (isAgentEnabled == false) {
    console.log('!!!!! Agent is disabled by server');
    return;
  }
  calculate();
